@extends('layouts.app', ['activePage' => 'summary1', 'title' => 'Summary Problem Prod. Report Line #1 - PCD', 'header' => __('Summary Problem Prod. Report Line #1')])
            ...
            <div class="col-md-12">
                ...
                <div class="card">
                    <div class="card-header card-header-primary">
                        <h4 class="card-title text-center text-uppercase font-weight-bold">{{ __('Summary Problem Prod. Report L#1') }}</h4>
                    </div>
                    <div class="card-body">
                        @if (auth()->user()->role == 'admin' || auth()->user()->role =='PIC CCR')
                            <div class="text-end">
                                <button type="button" class="btn btn-success btn-lg" data-toggle="modal" data-target="#importSummaryL1">
                        ...
                        <div class="row mt-3 px-1">
                            <div class="table-responsive">
                                <table class="table table-bordered" id="summaryReportL1Table">
                                    <thead>
                                    ...
                                    <tbody>
                                        @foreach ($summaryL1 as $summary)
                                            <tr class="text-center">
                                                <td>{{ $loop->iteration }}</td>
                                                <td>{{ Carbon\Carbon::parse($summary->tgl)->isoFormat('MMMM Y') }}</td>
                                                <td>
                                                    <a href="{{ route('download_summary1', $summary->id) }}" target="_blank" class="text-decoration-underline text-black">
                                                        <i class="fa-solid fa-file"></i> {{ $summary->filename }}
                                                    </a>
                                                </td>
                                            @if(auth()->user()->role == 'admin' || auth()->user()->role == 'PIC CCR')
                                                <td>
                                                    <form action="{{ route('delete_summary1', $summary->id) }}" method="POST">
                                                        @csrf
                        ...
                        <!-- modal -->
                        <div class="modal fade" id="importSummaryL1" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                            <div class="modal-dialog modal-dialog-centered" role="document">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title">Upload File Summary Report L#1</h5>
                                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                            <span aria-hidden="true">&times;</span>
                                        </button>
                                    </div>
                                    <form action="{{ route('import_summary1') }}" method="POST" enctype="multipart/form-data">
                                        {{ csrf_field() }}
...
@section('summaryReportL1Table')
    <script>
        $(document).ready( function () {
            $('#summaryReportL1Table').DataTable({
                dom: 'BflprtipS',
                buttons: [
                    'copy', 'excel', 'pdf'
                ]
...
@section('summaryReportL1DeleteConfirm')
